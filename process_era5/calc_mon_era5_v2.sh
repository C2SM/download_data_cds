...
# Purpose : calculate monthly means for e.g. derived variables calculated
#           using calc_variable_from_other_variable.py or any other case where
#           monthly calcualtion needs to be done

###-------------------------------------------------------
printf -v date '%(%Y-%m-%d_%H%M%S)T' -1
logfile="calc_mon_era5_v2_$date.log"
{
##-----------------------##
...
do
    ...
    do
        echo $YEAR
        mkdir -p ${outdir}/${VARI}/day/${grid}/${YEAR}
        mkdir -p ${outdir}/${VARI}/mon/${grid}/${YEAR}

        for MONTH in $(seq -w 01 12)
        do
            echo $MONTH
            name_mon=${outdir}/${VARI}/mon/${grid}/${YEAR}/${VARI}_mon_${data}_${YEAR}${MONTH}.nc
            name_day=${outdir}/${VARI}/day/${grid}/${YEAR}/${VARI}_day_${data}_${YEAR}${MONTH}.nc
